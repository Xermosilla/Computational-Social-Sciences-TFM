...
press release per year was properly scrapped, and I was able to create this final code with the 109 press releases.
...
Becasue many scrapped words that started with uppercase were stuck to the previous word
...
Adding a column whit the year to which each press release belongs, as index
...
```
```{r}
# ggsave("../plots/graph1.png", plot= graph1)
# dev.copy(png, "../plots/graph1.png")
# dev.off()

# ggsave("../plots/graph1.png", plot = wordcountswordcloud, width = 8, height = 8, dpi = 300)
```
...
```
```{r}
ggsave("../plots/graph2.png", plot = graph2, width = 8, height = 8, dpi = 300)
```
...
```
```{r}
ggsave("../plots/graph9a.png", plot = graph9a, width = 8, height = 8, dpi = 300)
ggsave("../plots/graph9b.png", plot = graph9b, width = 8, height = 8, dpi = 300)
```
...
```
```{r}
ggsave("../plots/graph10.png", plot = graph10, width = 8, height = 8, dpi = 300)
```
...
```
```{r}
ggsave("../plots/graph11a.png", plot = graph11a, width = 8, height = 8, dpi = 300)
```
...
```
```{r}
ggsave("../plots/graph11b.png", plot = graph11b, width = 8, height = 8, dpi = 300)
```
...
```
```{r}
# ggsave("../plots/graph12.png", plot = graph12, width = 8, height = 8, dpi = 300)
```

Barplot

```{r}
graph14 <- bigram_counts %>%
...
```
```{r}
ggsave("../plots/graph14.png", plot = graph14, width = 8, height = 8, dpi = 300)
```
...
```
```{r}
# ggsave("../plots/graph13.png", plot = graph13, width = 8, height = 8, dpi = 300)
```

Barplot

```{r}
graph15 <- trigram_counts %>%
...
Independently from the number of categories selected (10, 6 and 4) the word count top ten words, like "compañía", "millón", "porcentaje" or "euros", are repeated in most of the categories at the same time.
This result is probably due to the fact that those words are really frequent words in my press release data, consequently, those words do not provide discriminative inbformation for topic detection. Therefore, I am going to remove those words to apply LDA again, in order to identify more concrete topics.
...
After aplying LDA to conclusions can be extracted:
